...
for i, col in enumerate(symbol_cols):
    ...
    symbol = col.text_input(f"Symbol {i+1}", 
                            max_chars=1, 
...
num_states = state_col1.number_input("Number of States", 
                                     min_value=1, 
                                     max_value=16, 
                                     value="min", 
                                     step=1, 
...
transition_df = pd.DataFrame(
    columns,
    index=[f"q{i}" for i in range(num_states)]
)
...
def graph_callback():
    ...
    dfa_obj = DFA(alphabet=(tuple(edited_df)), 
...
st.button("Generate Graph", type="primary", on_click=graph_callback, key="submit_config")
...
def validate(test_input: str):
    ...
    st.session_state.dfa_graph_obj = st.session_state.dfa_obj.create_dfa(validation_trace=True)
...
if "dfa_obj" in st.session_state and st.session_state.dfa_obj:
    ...
    st.info("Fill out the DFA configuration and generate a graph to begin testing strings.", icon="ℹ️")
